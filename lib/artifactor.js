...
class Artifactor {
  ...
  _require(contractName) {
    ...
    switch (this.config.artifactType) {
      ...
      case "hardhat-v1":
        return this._hardhatArtifactor(contractName);
      case "ethpm":
      default:
        return this._truffleArtifactor(contractName);
    }
  ...
  /**
   * [DEPRECATED]
   * Hardhat artifact translator. Solc info (metadata) is attached to config
   * at the hardhat plugin
   * @param  {String} contractName
   * @return {Object}              egr artifact
   */
  _hardhatArtifactor(contractName) {
    const artifact = artifacts.require(contractName);
