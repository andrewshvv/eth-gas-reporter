...
class TransactionWatcher {
  ...
  async collectGasUsage(startBlock, endBlock, itStartBlock) {
    let gasUsed = 0;
    let currentBlock = startBlock;

    console.log("BOLOCKS:", startBlock, itStartBlock, endBlock);

    while (currentBlock <= endBlock) {
      let block = await this.provider.getBlock(currentBlock, true);

      if (block) {
        // Avoid gas used within `it` blocks
        if (currentBlock >= itStartBlock) {
  ...
  _collectMethodsData(transaction, receipt) {
    ...
    if (this.data.methods[id]) {
      ...
      console.log(receipt);
      console.log(
        this.data.methods[id].contract,
        this.data.methods[id].fnSig,
        receipt.gasUsed
      );
      console.log(
        "============================================================"
