...
class GasTable {
  ...
  generate(info) {
    ...
    _.forEach(info.methods, (data, methodId) => {
      ...
      if (data.gasData.length) {
        const total = data.gasData.reduce(
          (acc, datum) => acc.add(datum),
          new BigNumber.from(0)
        );
        stats.average = total
          .div(data.gasData.length)
          .toNumber()
          .toString();
      ...
      stats.numberOfCalls = colors.grey(data.gasData.length.toString());
    ...
    info.deployments.forEach(contract => {
      ...
      stats.average = total.div(contract.gasData.length).toNumber();
